	...
	class userlist extends general {
		...
		public function  insertDatastep2(){
			...
			if(($_POST['email'] == '' && $_POST['fullname'] == '' ) ) {
				...
				$expiredate = date('Y-m-d', strtotime("+14 day"));
				...
				if(count($fnuser) > 9){
					...
					foreach($fnuser as $fn){
                       /* $fn=trim($fn);
						...
						$getTeamid =  parent::select("*","tbl_team"," AND company_user_id = '".$_SESSION['register_id']."' ORDER BY tm_id asc limit 0,1");
						$teamArr = array(
							'tm_id'=>$getTeamid[0]['tm_id'],
							'user_id'=>$id,
							);
						parent::insert('tbl_team_detail',$teamArr);
