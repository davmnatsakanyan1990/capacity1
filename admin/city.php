...
// Configration 
	...
	$objPages = $load->includeclasses('city');
	$label = "city";
	$dclass   =  new  database();
	$gnrl =  new general;
...
if(isset($_REQUEST['script']) && $_REQUEST['script']=='delete') {
	// $chkParent = $dclass->select("COUNT(*) as cnt","tbl_area"," AND city_id=".$_REQUEST['id']); 
	 $chkParent = $dclass->select('COUNT(*) as cnt','tbl_product',' AND city_id ='.$_REQUEST['id']);
            ...
            <div class="portlet-body form">
              ...
              <form name="<?php echo $label;?>" id="<?php echo $label;?>" method="post"  enctype="multipart/form-data">
                <div class="control-group">
                  <label class="control-label valiclrBold">City Name *</label>
                  <div class="controls">
                  <input type="text" maxlength="150" name="city_name"  id= "city_name" size=30 value="<?php echo $row[0]['city_name'];?>" class="validate[required] span6 m-wrap">
                  </div>
                </div>
                
                <div class="control-group">
                  <label class="control-label valiclrBold">Country *</label>
                  <div class="controls">
                 <?php $country_list = $dclass->select("*",'tbl_country',' AND country_status="active"'); ?>
                              <select name="country_id" id="country_id" class="validate[required] span6 m-wrap" >
                              	<option value="" >Select Country</option>
								<?php foreach($country_list as $sec){ 
										if($sec['country_id'] == $row[0]['country_id']){
											$selected="selected=''selected";
										}else{
											$selected="";
										}
								?>
                                	<option value="<?php echo $sec['country_id'] ?>" <?php echo $selected; ?>  ><?php echo $sec['country_name'] ?></option>
								<?php } ?>
                              </select>
                  </div>
                </div>
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                <select name="city_status" class="validate[required] span6 m-wrap" id="city_status">
                                    <option value="" selected="selected" >Select Status</option>
                                    <option value="active"  <?php if($row[0]['city_status']=='active') {echo 'selected="selected"';} ?>  >Active</option>
                                    <option value="inactive"  <?php if($row[0]['city_status']=='inactive') {echo 'selected="selected"';} ?> >Deactive</option>
                                  </select>   
                ...
                <div class="control-group">
                  <label class="control-label">City Display Front?</label>
                  <div class="controls">
                  <?php if($row[0]['city_diaplay_front']=='yes'){
                      $checked = 'checked="checked"';
                  }else{
                      $checked = '';
                  }?>
                    <input type="checkbox" name="city_diaplay_front" class=""  <?php echo $checked; ?> value="yes"  />                  
               </div>
                </div>
                ...
                <thead>
                  <tr>
                    ...
                    <th class="hidden-480">Country Name</th>
                     <th class="hidden-480">Display In Front?</th>
                    <th class="center hidden-480">Action</th>
                   
                  </tr>
                </thead>
                <tbody>
                  <?php  $datalist = $dclass->select("*",'tbl_city',"ORDER BY city_id desc"); 
				  		foreach($datalist as $rows){
							$chkParent = $dclass->select('COUNT(*) as cnt','tbl_product',' AND city_id ='.$rows['city_id']);
							//$chkParent = $dclass->select("COUNT(*) as cnt","tbl_area"," AND city_id=".$rows['city_id']); 
				  ?>
                  <tr class="odd gradeX">
                    <td class="hidden-480">
                     <?php if($chkParent[0]['cnt'] == '0'){  ?>
                    <input type="checkbox" name="chk[]" class="checkboxes" value="<?php echo $rows['city_id']; ?>" />
                    <?php } ?>
                    </td>
                    <td class="hidden-480"><a href="<?php echo $label ?>.php?script=edit&id=<?php echo $rows['city_id']; ?>"><?php echo $rows['city_name']; ?></a></td>
                    <?php $Country = $dclass->select("country_name",'tbl_country',"AND country_id=".$rows['country_id']) ?>
        <td class="hidden-480" ><?php echo $Country[0]['country_name'];?></td>
        
        
        <td class="hidden-480"><?php echo ucfirst($rows['city_diaplay_front']); ?></td>
        
                    <td class="center hidden-480">
                    <?php
            if( $rows['city_status'] == 'inactive')
			{
				
				?>
                <span><a href="<?php echo $label;?>.php?script=activate&id=<?php echo $rows['city_id'];?>" onclick="return confirm('Are you sure you want to activate the selected records ?');"  title="deactivate" class="badge badge-roundless badge-important">Inactive</a></span>
				...
				?>	
            	<span><a href="<?php echo $label;?>.php?script=deactivate&id=<?php echo $rows['city_id'];?>" onclick="return confirm('Are you sure you want to deactivate the selected records ?');" title="activate" class="badge badge-roundless badge-success">Active</a></span>	    
                
                
                <?php
			}
			?>
            <span><a href="<?php echo $label;?>.php?script=edit&id=<?php echo $rows['city_id'];?>"><img src="<?php echo  IMAGEPATH; ?>edit-icon.png"  name="Edit" alt=""></a> </span> 
             <?php if($chkParent[0]['cnt'] == '0'){  ?>
            <span>
            
            <a href="<?php echo  $label.'.php?script=delete&id='.$rows['city_id'];?>" onclick="return confirm('Are you sure you want to delete this record?');" class="delfset" id="del1"> <img src="<?php echo  IMAGEPATH; ?>delete-icon.png"  name="Delete" alt=""></a>
            </span>
            <?php } ?>
