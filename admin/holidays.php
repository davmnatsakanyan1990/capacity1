...
// Configration 
	...
	$load  =  new loader();	
	$objPages = $load->includeclasses('holidays');
	$label = "holidays";
	$dclass   =  new  database();
	$gnrl =  new general;
  ...
  <?php  if (isset($_REQUEST['script'])  ||  $_REQUEST['script'] ==  'add' ||  $_REQUEST['script'] == 'edit'){
            ...
            <div class="portlet-body form">
              ...
              <form name="<?php echo $label;?>" id="<?php echo $label;?>" method="post"  enctype="multipart/form-data">
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                   <input type="text" maxlength="150" name="holi_title"  id= "holi_title" size=30 value="<?php echo $row[0]['holi_title'];?>" class="validate[required]">
                  </div>
                </div>
                ...
                <div class="control-group">
										...
										<div class="controls">
											<input class="m-wrap m-ctrl-medium date-picker validate[required]" value="<?php echo $row[0]['holi_start_date'] ?>" data-date-format="yyyy-mm-dd" name="holi_start_date" id="holi_start_date" readonly size="16" type="text" value="" />
										</div>
				</div>
                 ...
                 <div class="control-group">
										...
										<div class="controls">
											<input class="m-wrap m-ctrl-medium date-picker validate[required]" value="<?php echo $row[0]['holi_end_date'] ?>" data-date-format="yyyy-mm-dd" name="holi_end_date" id="holi_end_date" readonly size="16" type="text" value="" />
										</div>
				</div>
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                  <select name="holi_status" id="holi_status" class="validate[required] span6 m-wrap"  >
                		<option value="active" <?php if($row[0]['holi_status'] == 'active') echo "selected='selected'"; ?> >Active</option>
                		<option value="inactive" <?php if($row[0]['holi_status'] == 'inactive') echo "selected='selected'"; ?> >Inactive</option>
					</select>                
                	</div>
                </div>
    ...
    <?php }else{ ?>
            ...
            <table class="table table-striped table-bordered table-hover" id="sample_1_demo">
                ...
                <tbody>
                  <?php  $datalist = $dclass->select("*",'tbl_holidays',"ORDER BY holi_id desc"); 
				  		foreach($datalist as $rows){
				  ?>
                  <tr class="odd gradeX">
                    <td class="hidden-480"><input type="checkbox" name="chk[]" class="checkboxes" value="<?php echo $rows['holi_id']; ?>" /></td>
                    <td class="hidden-480"><a href="<?php echo $label ?>.php?script=edit&id=<?php echo $rows['holi_id']; ?>"><?php echo $rows['holi_title']; ?></a></td>
                    <td class="hidden-480"><?php echo $rows['holi_start_date']; ?></td>
                    <td class="hidden-480"><?php echo $rows['holi_end_date']; ?></td>
                <td class="center hidden-480">
            <?php
            if( $rows['holi_status'] == 'inactive')
			{
				
				?>
                <span><a href="<?php echo $label;?>.php?script=activate&id=<?php echo $rows['holi_id'];?>" onclick="return confirm('Are you sure you want to activate the selected records ?');" title="deactivate"class="badge badge-roundless badge-important">Inactive</a></span>
                <?php 	
			}
			else{
				?>	
            	<span><a href="<?php echo $label;?>.php?script=deactivate&id=<?php echo $rows['holi_id'];?>" onclick="return confirm('Are you sure you want to deactivate the selected records ?');" title="activate" class="badge badge-roundless badge-success">Active</a></span>	    
                <?php
			}
			?>
            <span><a href="<?php echo $label;?>.php?script=edit&id=<?php echo $rows['holi_id'];?>"><img src="<?php echo  IMAGEPATH; ?>edit-icon.png"  name="Edit" alt=""></a> </span> 
            <span><a href="<?php echo  $label.'.php?script=delete&id='.$rows['holi_id'];?>" onclick="return confirm('Are you sure you want to delete this record?');" class="delfset" id="del1"> <img src="<?php echo  IMAGEPATH; ?>delete-icon.png"  name="Delete" alt=""></a></span>
