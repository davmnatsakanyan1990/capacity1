...
// Configration 
	...
	$load  =  new loader();	
	$objPages = $load->includeclasses('email_template');
  ...
  <?php  if (isset($_REQUEST['script'])  ||  $_REQUEST['script'] ==  'add' ||  $_REQUEST['script'] == 'edit'){
            ...
            <div class="portlet-body form">
              ...
              <form name="<?php echo $label;?>" id="<?php echo $label;?>" method="post"  enctype="multipart/form-data">
                ...
                <div class="control-group">
                  <label class="control-label valiclrBold">Email Title *</label>
                  <div class="controls">
                  <input type="text" maxlength="150" name="email_title" <?php echo $readonly; ?>  id= "email_title" size=30 value="<?php echo $row[0]['email_title'];?>" class="validate[required] span6 m-wrap">
                  </div>
                </div>
                  
                <div class="control-group">
                  <label class="control-label valiclrBold">Email Subject *</label>
                  <div class="controls">
                  <input type="text" maxlength="150" name="email_subject"  id= "email_subject" size=30 value="<?php echo $row[0]['email_subject'];?>" class="validate[required] span6 m-wrap">
                  </div>
                </div>  
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                  <textarea name="email_body" id="emial_body" class="span12 ckeditor m-wrap" rows="6"><?php echo $row[0]['email_body'];?></textarea>
                  </div>
                </div>
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                 <select name="email_status" class="validate[required] span6 m-wrap" id="email_status">
                                    <option value="" selected="selected" >Select Status</option>
                                    <option value="1"  <?php if($row[0]['email_status']=='1') {echo 'selected="selected"';} ?>  >Active</option>
                                    <option value="0"  <?php if($row[0]['email_status']=='0') {echo 'selected="selected"';} ?> >Deactive</option>
                 </select>                 
                  </div>
                </div>
    ...
    <?php 
            ...
            <table class="table table-striped table-bordered table-hover" id="sample_1_demo">
                ...
                <tbody>
                  <?php  $datalist = $dclass->select("*",'tbl_email_template',"ORDER BY email_id desc"); 
				  		foreach($datalist as $rows){
				  ?>
                  <tr class="odd gradeX">
                    <td class="hidden-480"><input type="checkbox" name="chk[]" class="checkboxes" value="<?php echo $rows['email_id']; ?>" /></td>
                    <td class="hidden-480"><a href="<?php echo $label ?>.php?script=edit&id=<?php echo $rows['email_id']; ?>"><?php echo $rows['email_title']; ?></a></td>
                    <td class="hidden-480"><a href="<?php echo $label ?>.php?script=edit&id=<?php echo $rows['email_id']; ?>"><?php echo $rows['email_subject']; ?></a></td>
                    <td class="center hidden-480">
                    <?php
            if( $rows['email_status'] == '0')
			{
				
				?>
                <span><a href="<?php echo $label;?>.php?script=activate&id=<?php echo $rows['email_id'];?>" onclick="return confirm('Are you sure you want to activate the selected records ?');" title="deactivate"class="badge badge-roundless badge-important">Inactive</a></span>
                <?php 	
			}
			else{
				?>	
            	<span><a href="<?php echo $label;?>.php?script=deactivate&id=<?php echo $rows['email_id'];?>" onclick="return confirm('Are you sure you want to deactivate the selected records ?');" title="activate" class="badge badge-roundless badge-success">Active</a></span>	    
                <?php
			}
			?>
            <span><a href="<?php echo $label;?>.php?script=edit&id=<?php echo $rows['email_id'];?>"><img src="<?php echo  IMAGEPATH; ?>edit-icon.png"  name="Edit" alt=""></a> </span> 
            <span><a href="<?php echo  $label.'.php?script=delete&id='.$rows['email_id'];?>" onclick="return confirm('Are you sure you want to delete this record?');" class="delfset" id="del1"> <img src="<?php echo  IMAGEPATH; ?>delete-icon.png"  name="Delete" alt=""></a></span>
