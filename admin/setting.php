...
if(!empty($menuAccess))
{
	if(!$gnrl->checkGroupAccess('Social Media'))
	{
...
$load->includeother('left_sidebar');?>
  ...
  <?php  if (isset($_REQUEST['script'])  ||  $_REQUEST['script'] ==  'add' ||  $_REQUEST['script'] == 'edit'){
            ...
            <div class="portlet-body form">
              ...
              <form name="<?php echo $label;?>" id="<?php echo $label;?>" method="post"  enctype="multipart/form-data">
                ...
                <div class="control-group">
                  <label class="control-label valiclrBold">Option Title *</label>
                  <div class="controls">
                   <input type="hidden" name="id" id="id" value="<?php echo $id;?>" />
                   <input type="text" maxlength="150" name="v_name"  id= "v_name" size=30 value="<?php  echo  $v_name;?>" class="validate[required]"> <?php if($description != ""){ echo "(".$description.")";}?>
                  </div>
                </div>
                
               
                <div class="control-group">
                  <label class="control-label">Option Value Type</label>
                  <div class="controls">
                  <label class="radio"><input type="radio" name="type"  id= "type1" size=30 value="image" <?php if($type  ==  'image') echo 'checked="checked"';?> />Image</label>
                   <label class="radio"><input type="radio" name="type"  id= "type2" size=30 value="textbox" <?php if($type == 'textbox'){ echo 'checked="checked"';}else{  echo 'checked="checked"';} ?> />TextBox</label>    
                ...
                <!--  <input type="radio"  name="type"  id= "type1" size=30 value="image" <?php //if($type  ==  'image') echo 'checked="checked"';?> >Image &nbsp;&nbsp;&nbsp;
                  <input type="radio"  name="type"  id= "type2" size=30 value="textbox" <?php //if($type == 'textbox'){ echo 'checked="checked"';}else{  echo 'checked="checked"';} ?> >TextBox-->
                  
                  
                  </div>
                </div>
                
                <div class="control-group" id="textbox"  <?php if($type == 'textbox') echo 'style="display:table-row;"';?>>
                  <label class="control-label valiclrBold">Option Value *</label>
                  <div class="controls">
                  <?php /*?><input type="text"  name="l_values"  id= "l_values"  value="<?php echo strip_tags($l_values);?>" class="validate[required]"><?php */?>
                  
                   <textarea name="l_values" id="l_values" class="validate[required]" rows="6"><?php echo strip_tags($l_values);?></textarea>
                  
                  </div>
                </div>
                
                <div class="control-group" id="image" <?php if($type == 'image'){echo 'style="display:table-row;"';}else{echo 'style="display:none;"';}?>>
                  <label class="control-label valiclrBold">Option Value *</label>
                  <div class="controls">
                  <input type="file"  name="l_values"  id= "l_values" value="" class="validate[required]">
                  </div>
                </div>
    ...
    <?php }else{ ?>
            ...
            <table class="table table-striped table-bordered table-hover" id="sample_1_demo">
                <thead>
                  <tr>
                    ...
                    <th class="hidden-480">Option Title</th>
                    <th class="hidden-480">Option Value</th>
                    <th class="center hidden-480">Action</th>
                   
                  </tr>
                </thead>
                <tbody>
                  <?php  $datalist = $dclass->select("*",'tbl_setting',"ORDER BY id desc"); 
				  		foreach($datalist as $rows){
				  ?>
                  <tr class="odd gradeX">
                    <td class="hidden-480"><input type="checkbox" name="chk[]" class="checkboxes" value="<?php echo $rows['id']; ?>" /></td>
                    <td class="hidden-480"><a href="<?php echo $label ?>.php?script=edit&id=<?php echo $rows['id']; ?>"><?php echo $rows['v_name']; ?></a></td>
                    <td class="hidden-480"><?php echo $rows['l_values']; ?></td>                   
            ...
            <span><a href="<?php echo  $label.'.php?script=delete&id='.$rows['id'];?>" onclick="return confirm('Are you sure you want to delete this record?');" class="delfset" id="del1"> <img src="<?php echo  IMAGEPATH; ?>delete-icon.png"  name="Delete" alt=""></a></span>
