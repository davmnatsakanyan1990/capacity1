...
// Configration 
	...
	$load  =  new loader();	
	$objPages = $load->includeclasses('video');
	$label = "video";
	$dclass   =  new  database();
	$gnrl =  new general;
  ...
  <?php  if (isset($_REQUEST['script'])  ||  $_REQUEST['script'] ==  'add' ||  $_REQUEST['script'] == 'edit'){
            ...
            <div class="portlet-body form">
              ...
              <form name="<?php echo $label;?>" id="<?php echo $label;?>" method="post"  enctype="multipart/form-data">
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                      <input type="text" name="video_title" id="video_title" value="<?php echo $row[0]['video_title']?>" class="validate[required] span6 m-wrap">
                  </div>
                </div>
                ...
                <div class="control-group">
                  <label class="control-label">Video</label>
                  <div class="controls">
                  <textarea name="video_code" id="video_code" class="span12  m-wrap" rows="6"><?php echo $row[0]['video_code'];?></textarea>
                  </div>
                </div>
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                    <select name="video_status" class="validate[required] span6 m-wrap" id="video_status">
                        <option value="" selected="selected" >Select Status</option>
                        <option value="1" <?php if($row[0]['video_status'] == '1') {echo 'selected="selected"';} ?> >Active</option>
                        <option value="0"  <?php if($row[0]['video_status'] == '0') {echo 'selected="selected"';} ?>  >Inactive</option>
                    </select>                 
                  </div>
                </div>
    ...
    <?php 
            ...
            <table class="table table-striped table-bordered table-hover" id="sample_1_demo">
                ...
                <tbody>
                  <?php  $datalist = $dclass->select("*",'tbl_video',"ORDER BY video_id desc"); 
				  		foreach($datalist as $rows){
				  ?>
                  <tr class="odd gradeX">
                    <td class="hidden-480"><input type="checkbox" name="chk[]" class="checkboxes" value="<?php echo $rows['video_id']; ?>" /></td>
                    <td class="hidden-480"><a href="<?php echo $label ?>.php?script=edit&id=<?php echo $rows['video_id']; ?>"><?php echo $rows['video_title']; ?></a></td>
                    <td class="center hidden-480">
                    <?php
            if( $rows['status'] == '0')
			{?>
                <span><a href="<?php echo $label;?>.php?script=activate&id=<?php echo $rows['video_id'];?>" onclick="return confirm('Are you sure you want to activate the selected records ?');" title="deactivate"class="badge badge-roundless badge-important">Inactive</a></span>
                <?php 	
			}
			else{
				?>	
            	<span><a href="<?php echo $label;?>.php?script=deactivate&id=<?php echo $rows['video_id'];?>" onclick="return confirm('Are you sure you want to deactivate the selected records ?');" title="activate" class="badge badge-roundless badge-success">Active</a></span>	    
                <?php
			}
			?>
            <span><a href="<?php echo $label;?>.php?script=edit&id=<?php echo $rows['video_id'];?>"><img src="<?php echo  IMAGEPATH; ?>edit.gif"  name="Edit" alt=""></a> </span> 
            <span><a href="<?php echo  $label.'.php?script=delete&id='.$rows['video_id'];?>" onclick="return confirm('Are you sure you want to delete this record?');" class="delfset" id="del1"> <img src="<?php echo  IMAGEPATH; ?>delete-icon.png"  name="Delete" alt=""></a></span>
