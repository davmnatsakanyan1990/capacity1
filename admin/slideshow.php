...
// Configration 
	...
	$load  =  new loader();	
	$objPages = $load->includeclasses('slideshow');
	$label = "slideshow";
	$dclass   =  new  database();
	$gnrl =  new general;
  ...
  <?php  if (isset($_REQUEST['script'])  ||  $_REQUEST['script'] ==  'add' ||  $_REQUEST['script'] == 'edit'){
            ...
            <div class="portlet-body form">
              ...
              <form name="<?php echo $label;?>" id="<?php echo $label;?>" method="post"  enctype="multipart/form-data">
                ...
                <div class="control-group">
                  <label class="control-label valiclrBold">Title *</label>
                  <div class="controls">
                  <input type="text" maxlength="150" name="ss_title"  id= "ss_title" size=30 value="<?php echo $row[0]['ss_title'];?>" class="validate[required] span6 m-wrap">
                  </div>
                </div>
                
                
                <div class="control-group">
                  <label class="control-label valiclrBold">Discount Text *</label>
                  <div class="controls">
                  <input type="text" maxlength="150" name="ss_discount_text"  id= "ss_discount_text" size=30 value="<?php echo $row[0]['ss_discount_text'];?>" class="validate[required] span6 m-wrap">
                  </div>
                </div>
                
                <div class="control-group">
                  <label class="control-label valiclrBold">Description *</label>
                  <div class="controls">
                  <input type="text" maxlength="150" name="ss_desc"  id= "ss_desc" size=30 value="<?php echo $row[0]['ss_desc'];?>" class="validate[required] span6 m-wrap">
                  </div>
                </div>
					...
					<div class="controls">
                        <div class="fileupload fileupload-new" data-provides="fileupload">
                            <div class="input-append">
                                ...
                                <input type="file" name="ss_filename" id="ss_filename" class="default" />
                                </span>
                                <a href="#" class="btn fileupload-exists" data-dismiss="fileupload">Remove</a>
                            </div>
                            <?php if($_REQUEST['script'] == 'edit' && $row[0]['ss_filename'] != ''){ ?>
                                <img src="../upload/slideshow/<?php echo $row[0]['ss_filename'] ?>" width="120px"  />
                            <?php } ?>
                        </div>
                    </div>
				</div>
                <div class="control-group">
                  ...
                  <div class="controls">
                 <select name="ss_status" class="validate[required] span6 m-wrap" id="ss_status">
                                    <option value="" selected="selected" >Select Status</option>
                                    <option value="active"  <?php if($row[0]['ss_status']=='active') {echo 'selected="selected"';} ?>  >Active</option>
                                    <option value="inactive"  <?php if($row[0]['ss_status']=='inactive') {echo 'selected="selected"';} ?> >Deactive</option>
                 </select>                 
                  </div>
                </div>
    ...
    <?php }else{ ?>
            ...
            <table class="table table-striped table-bordered table-hover" id="sample_1_demo">
                ...
                <tbody>
                  <?php  $datalist = $dclass->select("*",'tbl_slideshow',"ORDER BY ss_id desc"); 
				  		foreach($datalist as $rows){
				  ?>
                  <tr class="odd gradeX">
                    <td class="hidden-480"><input type="checkbox" name="chk[]" class="checkboxes" value="<?php echo $rows['ss_id']; ?>" /></td>
                    <td class="hidden-480"><a href="<?php echo $label ?>.php?script=edit&id=<?php echo $rows['ss_id']; ?>"><?php echo $rows['ss_title']; ?></a></td>
                    <td class="hidden-480"><img src="../upload/slideshow/<?php echo $rows['ss_filename']; ?>" width="100px;"  /></td>
                   
                    <td class="center hidden-480">
                    <?php
            if( $rows['ss_status'] == 'inactive')
			{
				
				?>
                <span><a href="<?php echo $label;?>.php?script=activate&id=<?php echo $rows['ss_id'];?>" onclick="return confirm('Are you sure you want to activate the selected records ?');" title="deactivate"class="badge badge-roundless badge-important">Inactive</a></span>
                <?php 	
			}
			else{
				?>	
            	<span><a href="<?php echo $label;?>.php?script=deactivate&id=<?php echo $rows['ss_id'];?>" onclick="return confirm('Are you sure you want to deactivate the selected records ?');" title="activate" class="badge badge-roundless badge-success">Active</a></span>	    
                <?php
			}
			?>
            <span><a href="<?php echo $label;?>.php?script=edit&id=<?php echo $rows['ss_id'];?>"><img src="<?php echo  IMAGEPATH; ?>edit-icon.png"  name="Edit" alt=""></a> </span> 
            <span><a href="<?php echo  $label.'.php?script=delete&id='.$rows['ss_id'];?>" onclick="return confirm('Are you sure you want to delete this record?');" class="delfset" id="del1"> <img src="<?php echo  IMAGEPATH; ?>delete-icon.png"  name="Delete" alt=""></a></span>
