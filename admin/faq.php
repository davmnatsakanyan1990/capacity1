...
// Configration 
	...
	$load  =  new loader();	
	$objPages = $load->includeclasses('faq');
	$label = "faq";
	$dclass   =  new  database();
	$gnrl =  new general;
  ...
  <?php  if (isset($_REQUEST['script'])  ||  $_REQUEST['script'] ==  'add' ||  $_REQUEST['script'] == 'edit'){
            ...
            <div class="portlet-body form">
              ...
              <form name="<?php echo $label;?>" id="<?php echo $label;?>" method="post"  enctype="multipart/form-data">
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                      <textarea name="faq_question" id="faq_question" class=" validate[required] span12 ckeditor m-wrap" rows="6"><?php echo $row[0]['faq_question']?></textarea>
                  </div>
                </div>
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                  <textarea name="faq_answer" id="faq_answer" class=" validate[required] span12 ckeditor m-wrap" rows="6"><?php echo $row[0]['faq_answer'];?></textarea>
                  </div>
                </div>
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                      <input type="text" name="faq_order" id="faq_title" value="<?php echo $row[0]['faq_order']?>" class="validate[required,custom[integer]] span6 m-wrap">
                  </div>
                </div>   
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                    <select name="faq_status" class="validate[required] span6 m-wrap" id="status">
                        <option value="" selected="selected" >Select Status</option>
                        <option value="1" <?php if($row[0]['faq_status'] == '1') {echo 'selected="selected"';} ?> >Active</option>
                        <option value="0"  <?php if($row[0]['faq_status'] == '0') {echo 'selected="selected"';} ?>  >Inactive</option>
                    </select>                 
                  </div>
                </div>
    ...
    <?php 
            ...
            <table class="table table-striped table-bordered table-hover" id="sample_1_demo">
                ...
                <tbody>
                  <?php  $datalist = $dclass->select("*",'tbl_faq',"ORDER BY faq_id desc"); 
				  		foreach($datalist as $rows){
				  ?>
                  <tr class="odd gradeX">
                    <td class="hidden-480"><input type="checkbox" name="chk[]" class="checkboxes" value="<?php echo $rows['faq_id']; ?>" /></td>
                    <td class="hidden-480"><a href="<?php echo $label ?>.php?script=edit&id=<?php echo $rows['faq_id']; ?>"><?php echo $rows['faq_question']; ?></a></td>
                    <td class="center hidden-480">
                    <?php
            if( $rows['faq_status'] == '0')
			{?>
                <span><a href="<?php echo $label;?>.php?script=activate&id=<?php echo $rows['faq_id'];?>" onclick="return confirm('Are you sure you want to activate the selected records ?');" title="deactivate"class="badge badge-roundless badge-important">Inactive</a></span>
                <?php 	
			}
			else{
				?>	
            	<span><a href="<?php echo $label;?>.php?script=deactivate&id=<?php echo $rows['faq_id'];?>" onclick="return confirm('Are you sure you want to deactivate the selected records ?');" title="activate" class="badge badge-roundless badge-success">Active</a></span>	    
                <?php
			}
			?>
            <span><a href="<?php echo $label;?>.php?script=edit&id=<?php echo $rows['faq_id'];?>"><img src="<?php echo  IMAGEPATH; ?>edit-icon.png"  name="Edit" alt=""></a> </span> 
            <span><a href="<?php echo  $label.'.php?script=delete&id='.$rows['faq_id'];?>" onclick="return confirm('Are you sure you want to delete this record?');" class="delfset" id="del1"> <img src="<?php echo  IMAGEPATH; ?>delete-icon.png"  name="Delete" alt=""></a></span>
