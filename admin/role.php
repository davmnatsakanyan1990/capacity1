...
// Configration 
	...
	$objPages = $load->includeclasses('role');
	$label = "role";
	$dclass   =  new  database();
	$gnrl =  new general;
  ...
  <?php  if (isset($_REQUEST['script'])  ||  $_REQUEST['script'] ==  'add' ||  $_REQUEST['script'] == 'edit'){
            ...
            <div class="portlet-body form">
              ...
              <form name="<?php echo $label;?>" id="<?php echo $label;?>" method="post"  enctype="multipart/form-data">
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                  <input type="text" maxlength="150" name="r_title"  id= "r_title" size=30 value="<?php echo $row[0]['r_title'];?>" class="validate[required] span6 m-wrap">
                  </div>
                </div>
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                 <select name="r_status" class="validate[required] span6 m-wrap" id="r_status">
                                    <option value="" selected="selected" >Select Status</option>
                                    <option value="active"  <?php if($row[0]['r_status']=='active') {echo 'selected="selected"';} ?>  >Active</option>
                                    <option value="inactive"  <?php if($row[0]['r_status']=='inactive') {echo 'selected="selected"';} ?> >Deactive</option>
                 </select>                 
                  </div>
                </div>
    ...
    <?php }else{ ?>
            ...
            <table class="table table-striped table-bordered table-hover" id="sample_1_demo">
                ...
                <tbody>
                  <?php  $datalist = $dclass->select("*",'tbl_role',"ORDER BY r_id asc"); 
				  		foreach($datalist as $rows){
				  ?>
                  <tr class="odd gradeX">
                    <td class="hidden-480"><input type="checkbox" name="chk[]" class="checkboxes" value="<?php echo $rows['r_id']; ?>" /></td>
                    <td class="hidden-480"><a href="<?php echo $label ?>.php?script=edit&id=<?php echo $rows['r_id']; ?>"><?php echo $rows['r_title']; ?></a></td>
                    <td class="center hidden-480">
                    <?php
            if( $rows['r_status'] == 'inactive')
			{
				
				?>
                <span><a href="<?php echo $label;?>.php?script=activate&id=<?php echo $rows['r_id'];?>" onclick="return confirm('Are you sure you want to activate the selected records ?');" title="deactivate"class="badge badge-roundless badge-important">Inactive</a></span>
                <?php 	
			}
			else{
				?>	
            	<span><a href="<?php echo $label;?>.php?script=deactivate&id=<?php echo $rows['r_id'];?>" onclick="return confirm('Are you sure you want to deactivate the selected records ?');" title="activate" class="badge badge-roundless badge-success">Active</a></span>	    
                <?php
			}
			?>
            <span><a href="<?php echo $label;?>.php?script=edit&id=<?php echo $rows['r_id'];?>"><img src="<?php echo  IMAGEPATH; ?>edit-icon.png"  name="Edit" alt=""></a> </span> 
            <span><a href="<?php echo  $label.'.php?script=delete&id='.$rows['r_id'];?>" onclick="return confirm('Are you sure you want to delete this record?');" class="delfset" id="del1"> <img src="<?php echo  IMAGEPATH; ?>delete-icon.png"  name="Delete" alt=""></a></span>
