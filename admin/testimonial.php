...
// Configration 
	...
	$load  =  new loader();	
	$objPages = $load->includeclasses('testimonial');
	$label = "testimonial";
	$dclass   =  new  database();
	$gnrl =  new general;
  ...
  <?php  if (isset($_REQUEST['script'])  ||  $_REQUEST['script'] ==  'add' ||  $_REQUEST['script'] == 'edit'){
            ...
            <div class="portlet-body form">
              ...
              <form name="<?php echo $label;?>" id="<?php echo $label;?>" method="post"  enctype="multipart/form-data">
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                      <input type="text" name="tmn_user" id="tmn_user" value="<?php echo $row[0]['tmn_user']?>" class="validate[required] span6 m-wrap">
                  </div>
                </div>
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                  <textarea name="tmn_description" id="description" class="span12 ckeditor m-wrap" rows="6"><?php echo $row[0]['tmn_description'];?></textarea>
                  </div>
                </div>
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                      <input type="file" name="tmn_image" id="tmn_image" class="">
                      <?php if($row[0]['tmn_img'] != '' ){?><img src="../upload/testimonial/<?php echo $row[0]['tmn_img'];?>" height="100px" width="50px"><?php }?> 
                      <input type="hidden" name="tmn_image_edit" value="<?php echo $row[0]['tmn_img'];?>">
                  </div>
                </div>
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                      <input type="text" name="tmn_order" id="tmn_title" value="<?php echo $row[0]['tmn_order']?>" class="validate[required,custom[integer]] span6 m-wrap">
                  </div>
                </div> 
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                    <select name="tmn_status" class="validate[required] span6 m-wrap" id="status">
                        <option value="" selected="selected" >Select Status</option>
                        <option value="1" <?php if($row[0]['tmn_status'] == '1') {echo 'selected="selected"';} ?> >Active</option>
                        <option value="0"  <?php if($row[0]['tmn_status'] == '0') {echo 'selected="selected"';} ?>  >Inactive</option>
                    </select>                 
                  </div>
                </div>
    ...
    <?php 
            ...
            <table class="table table-striped table-bordered table-hover" id="sample_1_demo">
                ...
                <tbody>
                  <?php  $datalist = $dclass->select("*",'tbl_testimonial',"ORDER BY tmn_id desc"); 
				  		foreach($datalist as $rows){
				  ?>
                  <tr class="odd gradeX">
                    <td class="hidden-480"><input type="checkbox" name="chk[]" class="checkboxes" value="<?php echo $rows['tmn_id']; ?>" /></td>
                    <td class="hidden-480"><a href="<?php echo $label ?>.php?script=edit&id=<?php echo $rows['tmn_id']; ?>"><?php echo $rows['tmn_user']; ?></a></td>
                    <td class="hidden-480"><img src="../upload/testimonial/<?php echo $rows['tmn_img']; ?>" width="50" height="100"/></td>
                    <td class="center hidden-480">
                    <?php
            if( $rows['tmn_status'] == '0'){ ?>
                <span><a href="<?php echo $label;?>.php?script=activate&id=<?php echo $rows['tmn_id'];?>" onclick="return confirm('Are you sure you want to activate the selected records ?');" title="deactivate"class="badge badge-roundless badge-important">Inactive</a></span>
            <?php }else{ ?>	
            	<span><a href="<?php echo $label;?>.php?script=deactivate&id=<?php echo $rows['tmn_id'];?>" onclick="return confirm('Are you sure you want to deactivate the selected records ?');" title="activate" class="badge badge-roundless badge-success">Active</a></span>	    
            <?php } ?>
            <span><a href="<?php echo $label;?>.php?script=edit&id=<?php echo $rows['tmn_id'];?>"><img src="<?php echo  IMAGEPATH; ?>edit-icon.png"  name="Edit" alt=""></a> </span> 
            <span><a href="<?php echo  $label.'.php?script=delete&id='.$rows['tmn_id'];?>" onclick="return confirm('Are you sure you want to delete this record?');" class="delfset" id="del1"> <img src="<?php echo  IMAGEPATH; ?>delete-icon.png"  name="Delete" alt=""></a></span>
